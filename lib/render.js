...
function render(file, enc, cb) {
  try {
    ...
    if (page.body.indexOf('{{#*inline') !== -1 || page.body.indexOf('{{~#*inline') !== -1) {
      ...
      var inlinePartialRegex = /{{\~{0,1}#\*inline[\s]+(?<quote>[\"']{1})(?<name>layout-[a-zA-Z_-]*)\k<quote>\s*}}(?<body>[.\s\S]*?){{\/inline\s*\~{0,1}}}/gm;
      ...
      while (inlinePartialMatch = inlinePartialRegex.exec(page.body)) {//this way can get all match and group
